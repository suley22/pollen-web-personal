...
export function StatusInfo({ statusInfo, employerProfile }) {
  ...
  return (
    statusInfo && (
      <Card
        className={`mb-6 border-l-4 ${statusInfo.borderColor} ${statusInfo.bgColor}`}
      >
        <CardContent className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p
                className={`${statusInfo.textColor} text-sm`}
              ...
              {statusInfo.feedback && (
                <div className="bg-white border border-red-200 rounded-lg p-4 mt-3">
                  ...
                  <button
                    onClick={() =>
                      router.push(
                        "/employer-messages?conversation=pollen-team&feedback=profile-review",
                      )
                    }
              ...
              {employerProfile?.updated_at &&
                statusInfo.type === "changes_pending" && (
                  <p
                    className={`${statusInfo.textColor} text-xs mt-1 opacity-75`}
                    style={{ fontFamily: "Poppins" }}
                  >
                    Last updated:{" "}
                    {new Date(employerProfile.updated_at).toLocaleDateString(
                      "en-GB",
                      {
                        day: "numeric",
                        month: "long",
